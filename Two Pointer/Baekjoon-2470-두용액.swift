//
//  Baekjoon-2470-두용액.swift
//  AlgorithmProject
//
//  Created by yurim on 2021/06/25.
//
/* 투포인터) 두 용액
...
func twoSolutions() {
    ...
    while start < end {
        ...
        if abs(sum) < min {
            min = abs(sum)
            result1 = values[start]
            result2 = values[end]
        }
        if sum < 0  { start += 1 }  // 0보다 작으면 0에 가까워지도록 start를 이동
        else        { end -= 1 }    // 0보다 크면 0에 가까워지도록 end를 이동
    }
