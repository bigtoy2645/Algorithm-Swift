//
//  Programmers2-다리를지나는트럭.swift
//  AlgorithmProject
//
...
class Truck {
    func solution(_ bridge_length:Int, _ weight:Int, _ truck_weights:[Int]) -> Int {
        var bridge = [Int]()
        var current_weights = 0
        var time = 0, i = 0, count = 0
        
        while count < truck_weights.count { // 다리를 지난 트럭 개수 비교
            time += 1
            
            // 큐에서 다리를 지난 트럭 제거
            if bridge.count == bridge_length {
                let removeWeight = bridge.removeFirst()
                if removeWeight > 0 {
                    current_weights -= removeWeight
                    count += 1
                }
            }
            // 다리를 건널 수 있으면 큐에 추가
            if i < truck_weights.count, current_weights + truck_weights[i] <= weight {
