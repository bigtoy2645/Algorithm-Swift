//
//  Baekjoon-1966-프린터큐.swift
//  AlgorithmProject
//
//  Created by yurim on 2021/06/16.
//
/* 자료구조) 프린터 큐
 프린터 기기는 여러분이 인쇄하고자 하는 문서를 인쇄 명령을 받은 ‘순서대로’, 즉 먼저 요청된 것을 먼저 인쇄한다.
 여러 개의 문서가 쌓인다면 Queue에 쌓여 인쇄가 된다. 하지만 상근이는 새로운 프린터기 내부 소프트웨어를 개발하였는데, 이 프린터기는 다음과 같은 조건에 따라 인쇄를 하게 된다.
...
func printerQueue() {
    ...
    for _ in 0..<T {
        ...
        let N = NM[0], M = NM[1]
        var priorities = readLine()!.split(separator: " ").map { Int(String($0))! }
        
        var count = 0, myIndex = M
        
        while N != count {
            if priorities[0] == priorities.max() {
                count += 1
                priorities.removeFirst()
                if myIndex == 0 { break } else { myIndex -= 1 }
            } else {
                priorities.append(priorities.removeFirst())
                if myIndex == 0 { myIndex = priorities.count - 1 }
                else            { myIndex -= 1 }
            }
        }
